\section{El lenguaje ${S}^{\Sigma}$}
  ...
  % \end{theorem}
  % \begin{proof}
  %   \PN Sea $f: S \subseteq \omega^{n} \times \Sigma^{\ast m} \rightarrow O$ es computada por $\mathcal{P} \in
  ...
  %
  %   \PN Luego $h \in R_{0}^{\Sigma} = PR_{0}^{\Sigma}$, es decir $h \in \{Suc, Pred, C_{0}^{0,0}, C_{\varepsilon}^{0,0}
  %   \} \cup \{d_{a}: a \in \Sigma\} \cup \{p_{j}^{n,m} : 1 \leq j \geq n+m\}$. Por lo tanto, $f$ es $\Sigma$-computable.
  %
  %   \vspace{3mm}
  ...
  %
  %   \PN Sea $h \in \mathrm{R}_{k+1}^{\Sigma} - \mathrm{R}_{k}^{\Sigma}$. Existen varios casos, supongamos $\Sigma =
  %   \{@, \$\}$, probaremos el caso $h=R(f,\mathcal{G})$, con:
  %
  %   \begin{eqnarray*}
  %       f &:& S_{1 }\times \dotsc \times S_{n} \times L_{1} \times \dotsc \times L_{m} \rightarrow \SIGMA \\
  %       \mathcal{G}_{@} &:& S_{1} \times \dotsc \times S_{n} \times L_{1} \times \dotsc \times L_{m} \times \SIGMA
  ...
  % % Lemma 57: Sin prueba.
  % \begin{lemma}
  %   \PN Sea $\Sigma$ un alfabeto cualquiera. Las funciones $S$ y $\overline{\ \;}$ son $(\Sigma \cup \Sigma_{p})$-PR.
  % \end{lemma}
  %
  % % Lemma 58: Sin prueba.
  % \begin{lemma}
  ...
  % \end{lemma}
  % \begin{proof}
  %   \PN Para simplificar la prueba asumiremos que $\Sigma =\{@,\&\}$. Ya que $\mathrm{Ins}^{\Sigma}$ es unión de los
  ...
  %   \end{eqnarray*}
  %
  %   \PN solo debemos probar que $L_{1}, \dotsc, L_{15}$ son $(\Sigma \cup \Sigma_{p})$-PR, como las pruebas son
  ...
  %   \]
  %
  %   \PN es $(\Sigma \cup \Sigma_{p})$-PR. Primero nótese que $\alpha \in L_{11}$ si y solo si existen $k, m \in
  %   \mathbb{N}$ tales que:
  %
  %   \[
  ...
  %
  %   \[
  %     (\exists k \in \mathbb{N})_{k\leq 10^{\left\vert \alpha \right\vert}}(\exists m \in \mathbb{N})_{m \leq
  %     10^{\left\vert \alpha \right\vert}} \; \alpha = \mathrm{IFP}\bar{k}\mathrm{BEGINS}@\mathrm{GOTOL}\bar{m}
  %   \]
  %
  %   \PN Sea
  %
  ...
  %   (\Sigma \cup \Sigma_{p})^{\ast}$. Nótese que
  %
  %   \[
  ...
  %   \]
  %
  %   \PN lo cual nos dice que $P$ es $(\Sigma \cup \Sigma_{p})$-PR. Nótese que
  %
  %   \[
  %     \chi_{L_{11}} = \lambda \alpha \left[(\exists k \in \mathbb{N})_{k\leq 10^{\left\vert \alpha \right\vert}}(\exists
  %     m \in \mathbb{N})_{m\leq 10^{\left\vert \alpha \right\vert}} \; P(m,k,\alpha)\right]
  %   \]
  %
  %   \PN Utilizando dos veces el \textbf{Lemma 39} veremos que $\chi_{L_{11}}$ es $(\Sigma \cup \Sigma_{p})$-PR. Sea
  %
  %   \[
  %     Q = \lambda k\alpha \left[(\exists m \in \mathbb{N})_{m\leq 10^{\left\vert \alpha \right\vert}}\; P(m,k,\alpha)
  %     \right]
  %   \]
  %
  ...
  %     \left[10^{\left\vert \alpha \right\vert}\right] \circ p_{2}^{1,1},p_{1}^{1,1},p_{2}^{1,1})
  %   \]
  %
  %   \PN lo es. Ya que
  %
  ...
  %   \]
  %
  %   \PN en forma similar aplicando el \textbf{Lemma 39} obtenemos que $\chi_{L_{11}}$ es $(\Sigma \cup \Sigma_{p})$-PR.
  %
  ...
  %
  %   \[
  %     \chi _{L_{15}}=\lambda \alpha \left[ (\exists k\in \mathbb{N})_{k\leq 10^{\left\vert \alpha \right
  ...
  % \end{lemma}
  % \begin{proof}
  %   \PN Sea $<$ un orden total estricto sobre $\Sigma \cup \Sigma_{p}$. Sea $L = \{ \mathrm{L}\bar{k}:k\in \mathbb{N}\}
  %   \cup \{\varepsilon \}$. Veamos que $L$ es $\Sigma \cup \Sigma_{p}$-PR. Notar que:
  %
  %   \[
  %     \alpha \in L \Leftrightarrow \alpha = \varepsilon \vee (\exists k \in \mathbb{N}) \; \alpha = \mathrm{L}\bar{k}
  %   \]
  %
  %   \PN Dado que cuando existe $k$ tenemos que $\bar{k}$ es una subpalabra de $\alpha $, el \textbf{Lemma 58} nos dice
  ...
  %     \\
  %     F &=& \lambda \alpha \left[(\exists k \in \mathbb{N})_{k \leq 10^{\lvert \alpha \rvert}} \; Q(k,\alpha)\right] \\
  ...
  %     &\Rightarrow& F \text{ es } (\Sigma \cup \Sigma_{p})-PR
  %   \end{eqnarray*}
  %
  %   \PN Luego:
  %
  %   \[
  %     \chi_{L} = \lambda \alpha \left[\alpha = \varepsilon\right] \vee F
  %   \]
  %
  %   \PN es $\Sigma$-PR y por lo tanto L lo es.
  %
  %   \PN Sea
  %   \[
  ...
  %     P_3 &=& \lambda \alpha\beta \left[\alpha \neq \beta\right] \circ (\lambda i\alpha \left[\left[\alpha\right]_i
  %       \right] \circ (p_{2}^{0,2},C_{1}^{0,2}), C_{L}^{0,2}) \\
  ...
  %     (\lambda \alpha \left[\lvert \alpha \rvert\right] \circ p_{1}^{0,2},p_{1}^{0,2},p_{1}^{0,2})
  %   \]
  %
  %   \PN lo es. Por lo tanto $P$ es $(\Sigma \cup \Sigma_{p})$-PR.
  %
  %   \vspace{3mm}
  %   \PN Nótese que cuando $I \in \mathrm{Ins}^{\Sigma}$ tenemos que $P(I,\alpha)=1$ sii $\alpha=Bas(I)$, además notar
  %   que $Bas = M^{<}\left(P\right)$ por lo que para ver que $Bas$ es $(\Sigma \cup \Sigma_{p})$-PR, solo resta encontrar
  %   un función cota $(\Sigma \cup \Sigma_{p})$-PR y $(\Sigma \cup \Sigma_{p})$-total para $Bas$. Notar que $\lvert
  %   Bas(I)\rvert \leq \lvert I \rvert = p_{1}^{0,1}(I)$, para cada $I \in \mathrm{Ins}^{\Sigma}$.
  %   Finalmente note que
  %
  %   \[
  ...
  %
  %
  %   Veremos que $\Phi _{\#}^{n,m}$ es $(\Sigma \cup \Sigma_{p})$-recursiva. Sea $H$ el predicado
  %   $(\Sigma \cup \Sigma_{p})$-mixto
  %
  %   \[
  %     \displaystyle \lambda t\vec{x}\vec{\alpha}\mathcal{P}\left[ i^{n,m}(t,x_{1},\dotsc,x_{n}, \alpha _{1},\dotsc,\alpha _{m},
  ...
  %   $(\Sigma \cup \Sigma_{p})$-PR., $H$ lo es. Notar que $D_{M(H)}=D_{\Phi _{\#}^{n,m}}$.
  ...
  %   $((x_{1},\dotsc,x_{n},0,0,\dotsc),(\alpha _{1},\dotsc,\alpha _{m},\varepsilon ,\varepsilon ,\dotsc))$.
  %   Ya que $H$ es $(\Sigma \cup \Sigma_{p})$-PR., tenemos que $M(H)$ es $(\Sigma \cup \Sigma_{p})$-r..
  ...
  %   \[
  %     \displaystyle \Phi _{\#}^{n,m}=E_{\#1}^{n,m}\circ \left( M(H),p_{1}^{n,m+1},\dotsc,p_{n+m+1}^{n,m+1}\right)
  %   \]
  %   Ya que la función $E_{\#1}^{n,m}$ es $(\Sigma \cup \Sigma_{p})$-r., lo es $ \Phi _{\#}^{n,m}$.
  %
  %   \begin{tabular}{|c|} \hline $\Phi_{\ast}^{n,m}$ es $(\Sigma \cup \Sigma_{p})$-recursiva \\\hline \end{tabular}
  % \end{proof}
  %
  ...
  %   \cup \Sigma_{p}$, es decir que tienen dominio $\omega^{n} \times (\Sigma \cup \Sigma_{p})^{\ast m}$. Ya que, por el
  %   \textbf{Theorem 65} $\Phi_{\#}^{n,m}, \Phi_{\ast}^{n,m}$ son $(\Sigma \cup \Sigma_{p})$-recursivas tenemos que $f$
  %   es $(\Sigma \cup \Sigma_{p})$-recursiva. Luego, utilizando el \textbf{Theorem 51} tenemos:
  %
  %   \begin{equation*}
	% 	  	\left.
	% 	  	\begin{array}{l}
  ...
  % \end{lemma}
  % \begin{proof}
  %   \PN Vamos a probar solo el caso en que $k = 2$ y $O = \SIGMA$. Sean $\mathcal{P}_{1}$ y $ \mathcal{P}_{2}$ programas
  %   que computen las funciones $f_{1}$ y $f_{2}$, respectivamente. Sean:
  %
  %   \begin{eqnarray*}
  ...
  %   $(\Sigma \cup \Sigma_{p})$-PR. Además, utilizando el \textbf{Theorem 51} tenemos que son $\Sigma $-PR. También
  %   notese que $D_{M((P_{1} \vee P_{2}))} = D_{f_{1}} \cup D_{f_{2}}$. Definamos:
  %
  %   \begin{eqnarray*}
  %     g_{1} & =& \lambda \vec{x}\vec{\alpha}\left[E_{\ast 1}^{n,m} (M \left(P_{1} \vee P_{2}\right)(\vec{x},
  %       \vec{\alpha}),\vec{x},\vec{\alpha},\mathcal{P}_{1})^{P_{1} (M (P_{1} \vee P_{2})(\vec{x},\vec{\alpha}),\vec{x},
  %       \vec{\alpha})}\right] \\
  %     g_{2} & =& \lambda \vec{x}\vec{\alpha}\left[E_{\ast 1}^{n,m} (M \left(P_{1} \vee P_{2}\right)(\vec{x},
